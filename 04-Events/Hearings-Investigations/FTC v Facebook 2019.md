---
date: 2026-05-16
description: $5 billion FTC fine on Facebook July 2019 - largest US privacy penalty ever
tags: [event, topic/facebook-data, topic/cambridge-analytica, era/2018-2020, jurisdiction/us]
type: event
event_date: 2019-07-24
location: United States
actors: [Federal Trade Commission, Facebook, Meta Platforms]
outcome: $5B fine; 20-year FTC consent decree; new Privacy Committee; affirmative consent required for facial recognition
status: closed
---

# FTC v Facebook 2019

## Summary

On **July 24, 2019**, the US Federal Trade Commission voted 3-2 to approve a record **$5 billion penalty against [[Facebook]]** â€” the largest privacy-related fine in US history â€” for violating its 2012 consent decree on user privacy. The FTC complaint alleged:

- Facebook privacy settings had falsely claimed to let users restrict sharing to "Only Friends" while still exposing data to third-party apps installed by friends
- Failed to screen developers or apps before granting access to user data
- Turned on facial recognition by default while claiming user control
- Collected phone numbers for two-factor authentication but used them for advertising without disclosure

Settlement created a 20-year consent decree, independent Privacy Committee on Facebook's board, and affirmative-consent requirement for facial recognition. Commissioner Rohit Chopra (dissenting) called the deal toothless; Commissioner Rebecca Slaughter argued the FTC should have sued [[Mark Zuckerberg]] personally. Also in July 2019, Facebook agreed a $100M SEC settlement for misleading investors about the same data exposure. ([[Source Wikipedia - Facebook-CA scandal]])

## Sources

- [[Source Wikipedia - Facebook-CA scandal]]
- [[Source Wikipedia - Cambridge Analytica]]

## Vault context and open research threads

This event matters to the vault because it is either a documented deployment of CA, SCL, or successor-org methodology against a real electorate, or because it produced primary evidence (regulator action, court filing, leak, sworn testimony) that the rest of the vault depends on.

Use this node to anchor: what actually happened (chronology with dates), who was on which side (clients, operators, targets), what was deployed (data collection, microtargeting, dark ads, deepfakes, voter suppression, narrative manipulation), and what is known vs alleged vs hypothesized.

## Open research threads

- Primary sources: which court filings, regulator reports, leaked documents, or sworn statements are directly tied to this event?
- Personnel: which named individuals from the vault appeared on the operator side? Cross-link them.
- Money: how was this event funded, and through what corporate or PAC structures?
- Outcome: what was the measured (not claimed) effect on the political outcome, and what is the evidence?
- Replication: which later events used the same playbook? Link forward.

## See also

- [[Events MOC]]
- [[Master Timeline]]
- [[Vault MOC]]
