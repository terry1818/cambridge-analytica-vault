---
date: 2026-05-16
description: 2018 scandal exposing harvest of up to 87M Facebook profiles by Cambridge Analytica via Aleksandr Kogan's "This Is Your Digital Life" app
tags: [event, topic/cambridge-analytica, topic/facebook-data, topic/election-interference, era/2014-2016, era/2016-2018, jurisdiction/uk, jurisdiction/us]
type: event
event_date: 2018-03-17 # major public break date
location: UK / US (data globally affected)
actors: [Cambridge Analytica, SCL Group, Facebook, Aleksandr Kogan, Global Science Research, Christopher Wylie, Brittany Kaiser, Mark Zuckerberg, Alexander Nix]
outcome: CA bankruptcy; $5B FTC fine on Facebook; Â£500K UK ICO fine; $725M class settlement
status: closed
wikipedia_url: https://en.wikipedia.org/wiki/Facebook%E2%80%93Cambridge_Analytica_data_scandal
aliases: ["Facebook-Cambridge Analytica Scandal"]
---

# Facebookâ€“Cambridge Analytica Data Scandal

## Summary

In the 2010s, [[Cambridge Analytica]] harvested personal data of up to **87 million Facebook users** without informed consent, via an app called **"This Is Your Digital Life"** built by [[Aleksandr Kogan]]'s firm [[Global Science Research (GSR)]] in 2013. The data was used to build psychographic profiles for political ad targeting â€” notably for the 2016 [[Ted Cruz]] and [[Donald Trump]] presidential campaigns. First reported in December 2015 by [[Harry Davies]] in [[The Guardian]]; broke open globally on March 17, 2018 via simultaneous reporting from [[The Guardian]]/[[The Observer]] and [[The New York Times]] based on disclosures by whistleblower [[Christopher Wylie]]. ([[Source Wikipedia - Facebook-CA scandal]])

## When & where

- **First reported:** 2015-12-11 by Harry Davies in The Guardian
- **Public eruption:** 2018-03-17 (Wylie disclosures)
- **Scope:** Global; ~70.6M US users affected; 6.7M California, 5.6M Texas, 4.3M Florida; 310K+ Australia; 562K India; 1M+ Philippines

## Actors

### Data harvesters
- [[Aleksandr Kogan]] â€” Cambridge University psychologist; built TIYDL app via [[GSR]]
- [[Global Science Research (GSR)]] â€” Kogan's company that built the app
- [[Cambridge Analytica]] â€” bought the data from GSR
- [[SCL Group]] â€” CA's parent
- [[Alexander Nix]] â€” CA CEO

### Whistleblowers / disclosure
- [[Christopher Wylie]] â€” primary whistleblower, ex-Director of Research at CA
- [[Brittany Kaiser]] â€” whistleblower; later HindsightFiles disclosures (2020)
- [[Shahmir Sanni]] â€” separate Vote Leave whistleblower
- [[Carole Cadwalladr]] â€” Guardian/Observer journalist who coaxed Wylie to come forward
- [[Emma Briant]] â€” Essex academic; brought additional evidence to UK Parliament

### Platform
- [[Facebook]] / [[Meta]]
- [[Mark Zuckerberg]] â€” CEO; testified to US Congress April 2018

### Regulators
- [[Information Commissioner's Office (ICO)]] (UK)
- [[Federal Trade Commission (FTC)]] (US)
- [[Securities and Exchange Commission (SEC)]] (US)
- [[UK Parliament DCMS Committee]]
- [[US Senate Judiciary Committee]]

## Mechanism

1. [[Aleksandr Kogan]] built a personality-quiz app, "This Is Your Digital Life"
2. ~270,000 users downloaded and consented (paid $4 via Qualtrics)
3. App exploited [[Facebook Graph API v1.0]] **friends-data permission** â€” collected profile data of every Facebook friend of each user (no friends' consent)
4. Average user had ~300 friends â†’ up to **87M profiles harvested**
5. Kogan/GSR shared data with [[Cambridge Analytica]], breaching Facebook ToS
6. Facebook deprecated friends-data API in **April 2014**; shut it down entirely **April 2015** â€” but data already harvested remained in developers' hands
7. CA used data for psychographic profiling, then targeted political messaging

## Data used by

- [[Ted Cruz Campaign 2016]] â€” paid CA $5.8M
- [[Donald Trump Campaign 2016]] â€” used during primaries and summer; later switched to RNC data
- [[Make America Number 1 Super PAC]] â€” pro-Trump (Mercer-funded) attack ads on [[Hillary Clinton]]
- [[Leave.EU]] (alleged; ICO later found "not involved beyond initial enquiries")
- [[Trinidad 2010 Do So Campaign]] â€” voter suppression of Afro-Trinidadians

## Russia / GRU connection thread

- ICO investigation found Kogan IP addresses and servers in Russia and associated countries
- Cambridge University Psychometrics Centre had earlier app accessed from Russian IPs linked to Tor entry point and alleged cyber attacks
- Kogan named on a grant at Saint Petersburg State University (cyberbullying research, post-2013 visit)
- Cambridge Analytica's data was of interest to Russian oil company [[Lukoil]] (confirmed by [[Christopher Wylie]])
- US polling/election data flowed via [[Sam Patten]] (CA contractor) â†’ [[Paul Manafort]] (Trump campaign manager) â†’ [[Konstantin Kilimnik]] (Russian agent, indicted)
- [[Alexander Nix]] reportedly approached [[Julian Assange]]/[[WikiLeaks]] for Clinton's 33,000 deleted emails

## Sequence of events

- `2012` â€” Facebook patents personality-prediction technology from social-network data
- `2013` â€” [[GSR]] / [[Aleksandr Kogan]] launches "This Is Your Digital Life"
- `2014-04` â€” Facebook deprecates friends-data API (Graph API v1.0 â†’ v2.0)
- `2014` â€” CA buys GSR data
- `2015-04` â€” Facebook shuts friends-data API entirely
- `2015-12-11` â€” [[Source Davies 2015-12-11 Guardian]] first major exposÃ© on Cruz/CA Facebook data use
- `2016-01` â€” Facebook receives signed certifications from Nix and Kogan that data was deleted (later proven false)
- `2016-11` â€” [[McKenzie Funk]] NYT Sunday Review follow-up
- `2016-12` â€” Grasseger/Krogerus Das Magazin (Swiss); later translated by Vice
- `2017-02-26` â€” Cadwalladr in The Observer "Robert Mercer: The big data billionaire waging war on mainstream media"
- `2017-05` â€” Cadwalladr in The Guardian "The Great British Brexit Robbery"
- `2017-03` â€” Mattathias Schwartz The Intercept
- `2018-03-17` â€” Wylie disclosures: simultaneous publication in [[The Guardian]] / [[The Observer]] and [[The New York Times]]
- `2018-03-19` â€” [[Channel 4 News]] broadcasts undercover Nix sting
- `2018-03-23` â€” Facebook bans CA; UK High Court grants ICO warrant
- `2018-03-25` â€” Zuckerberg publishes apology letter in newspapers
- `2018-03-26` â€” Facebook stock falls ~24%, $134B market cap loss
- `2018-04-10` â€” Zuckerberg testifies to US Congress
- `2018-04` â€” Facebook applies EU GDPR globally; establishes Social Science One
- `2018-05-01` â€” Cambridge Analytica files Chapter 7 bankruptcy
- `2018-05-16` â€” Senate Judiciary CA-focused hearing â€” testimony from [[Eitan Hersh]], Mark Jamison, Wylie
- `2018-07` â€” UK ICO announces intent to fine Facebook Â£500K (max under pre-GDPR law)
- `2019-02-18` â€” [[Source DCMS Final Report 2019]]
- `2019-03` â€” US AG for DC alleges Facebook knew of CA's "improper data-gathering practices" before Dec 2015
- `2019-07` â€” FTC $5B fine on Facebook (largest US privacy fine); 20-year settlement order
- `2019-07` â€” Facebook agrees $100M SEC settlement for misleading investors
- `2019-07-23` â€” FTC files complaint vs CA; Nix and Kogan accept 20-year restrictions
- `2019-10` â€” Facebook agrees Â£500K ICO fine
- `2020-01-04` â€” [[HindsightFiles]] release: 100,000+ docs from Brittany Kaiser, work in 68 countries
- `2020-11` â€” ICO closes Brexit inquiry: SCL/CA "not involved" in Brexit referendum
- `2022-12-23` â€” Meta agrees **$725M class-action settlement**

## Disputed facts

- **Effectiveness:** CA claimed transformative impact; many political scientists (Hersh, Nyhan) call psychographic effectiveness exaggerated. NYT March 2017: "Cambridge executives now concede that the company never used psychographics in the Trump campaign." `#claim/disputed`
- **Numbers:** CA initially claimed 30M; Facebook later confirmed up to 87M.
- **Brexit involvement:** Cadwalladr / Kaiser say CA worked for Leave.EU; CA denied; ICO (Nov 2020) confirmed no significant breach. `#claim/disputed`
- **Russia connection:** suggested by multiple threads (Lukoil interest, IP addresses, Patten/Manafort/Kilimnik); ICO's Denham found no Russian involvement in Brexit. `#claim/disputed`

## Consequences

- CA bankruptcy
- ~$134B Facebook market cap drop (recovered by May 2018)
- $5B FTC fine (Facebook)
- $100M SEC settlement (Facebook)
- Â£500K UK ICO fine (Facebook)
- $725M Meta class-action settlement (2022)
- 7-year UK director disqualification of [[Alexander Nix]] (2020)
- Sparked **#DeleteFacebook** movement (~400K tweets in 30 days)
- Accelerated EU GDPR awareness; Facebook applies GDPR globally
- Permanent shift in public discourse around social-media privacy and political microtargeting

## Open questions

- [ ] What did Facebook know, and when, before Dec 2015?
- [ ] What happened to the harvested data after "deletion" certifications?
- [ ] Full scope of Russian access via shared infrastructure with Cambridge Psychometrics Centre

## Sources

- [[Source Wikipedia - Facebook-CA scandal]]
- [[Source Davies 2015-12-11 Guardian]]
- [[Source Cadwalladr 2017-02-26 Observer Robert Mercer]]
- [[Source Cadwalladr 2017-05 Guardian Great British Brexit Robbery]]
- [[Source Rosenberg et al 2018-03-17 NYT]]
- [[Source Cadwalladr Graham-Harrison 2018-03-17 Guardian]]
- [[Source Channel 4 2018-03-19 Exposed]]
- [[Source Kang Frenkel 2018-04-04 NYT 87 Million]]
- [[Source DCMS Final Report 2019]]
- [[Source FTC Complaint Cambridge Analytica 2019]]
- [[Source Meta Settlement 2022 Reuters]]
- [[Source The Great Hack (2019)]]
- [[Source Mindfck (Wylie 2019)]]
- [[Source Targeted (Kaiser 2019)]]

## Post-2018 enforcement aftermath and the broader policy consequences

The Facebook-CA Data Scandal produced the deepest single privacy enforcement action in US history (the [[FTC v Facebook 2019]] $5 billion settlement) and substantial structural changes to the Facebook developer platform. The full enforcement-and-policy aftermath through 2024-2026:

**Regulatory consequences across jurisdictions.** US: the FTC settlement included structural privacy reforms, board-level privacy oversight, and ongoing FTC compliance reporting; SEC enforcement on disclosure questions produced an additional $100M penalty; multiple state-attorney-general settlements in 2022-2024 produced an additional $725M class-action settlement covering US Facebook users. UK: GBP 500,000 ICO fine (the pre-GDPR statutory maximum). EU: GDPR enforcement under the post-2018 regulatory regime has produced multiple subsequent Meta penalties for related data-processing-failure categories.

**Structural product changes.** Facebook deprecated Graph API v1 in 2015 (pre-CA-public-disclosure) and progressively tightened developer access through v2 and v3; the post-2018 acceleration of those changes included the elimination of friend-data access scopes entirely, the requirement of app-review for any data scope beyond basic profile information, and the introduction of the "Off-Facebook Activity" controls that gave users visibility into the data-collection happening via Facebook tracking pixels and SDKs on third-party websites and apps.

**Industry-wide effects.** The CA scandal substantially restructured the broader social-media-app-developer ecosystem. Many smaller apps that had relied on Facebook social-graph access lost data-access during the post-2018 platform tightening; the broader market for "social discovery" applications declined; the political-targeting market specifically shifted toward more direct first-party data collection and away from the platform-mediated data-access model that CA had exploited.

## What the scandal did and did not change about the underlying capability

The CA scandal eliminated the specific data-access vector that GSR/Kogan exploited (Graph API v1 friend-data exposure) but did not eliminate the underlying capability the vault is built to analyse. The 2024-2026 information environment still supports:

- Individual-level targeting at scale (now principally through Facebook's own internal Custom Audiences and Lookalike Audiences infrastructure, which gives advertisers comparable targeting capability without the third-party-developer-data-access exposure that CA used).
- Psychographic-style audience modelling (now substantially absorbed into Facebook's own machine-learning infrastructure and into commercial data-broker products that aggregate non-Facebook data sources).
- Cross-platform identity resolution (commercial vendors and the platform's own infrastructure both support this).
- Dark-ad deployment at scale (the Facebook Ad Library post-2018 transparency improvements have made the deployment partially visible but have not eliminated it).

The vault's analytical position: the CA scandal was the public-facing reckoning for an infrastructure that has continued operating in a more centralised form. The capability did not disappear; the third-party-developer access to that capability was substantially restricted, with the capability remaining inside the platforms themselves.

## See also

- [[Cambridge Analytica]]
- [[Facebook]]
- [[Facebook Graph API v1]]
- [[Aleksandr Kogan]]
- [[Christopher Wylie]]
- [[Carole Cadwalladr]]
- [[Mark Zuckerberg]]
- [[FTC v Facebook 2019]]
- [[ICO Investigation]]
- [[Hypothesis - Federal database supersedes CA model]]