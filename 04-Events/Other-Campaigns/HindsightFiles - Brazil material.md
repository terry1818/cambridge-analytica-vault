---
date: 2026-05-16
description: Analysis of HindsightFiles documents covering CAs Brazil operations
tags: [event, topic/cambridge-analytica, era/2016-2018, jurisdiction/other]
type: event
location: Brazil
actors: [Cambridge Analytica, SCL Group, Brittany Kaiser]
status: closed
---

# HindsightFiles â€” Brazil Material

## What HindsightFiles documents cover

[[Brittany Kaiser]]'s [[HindsightFiles]] release included a substantial chunk of material on **CA's Brazilian operations** â€” material covered most extensively by **Folha de SÃ£o Paulo** following the January 2020 release.

### Key disclosures

- Documents describe CA's outreach to Brazilian political parties **before the 2018 presidential election** (won by [[Jair Bolsonaro]])
- Material indicates discussions with multiple parties (consistent with CA's pattern of pitching multiple sides)
- Documents reference data-sourcing arrangements with Brazilian data brokers and consumer-research firms
- Some material on subnational/regional electoral work in addition to federal

## Significance

Brazil is significant because:
- 2018 Bolsonaro victory was a major populist-right milestone
- [[Steve Bannon]] (via "The Movement") advised Bolsonaro
- The Brazilian election is widely cited as having significant disinformation/WhatsApp-driven elements
- HindsightFiles establishes that **CA was actively pitching in the same election cycle**

## Open questions

- [ ] Which specific Brazilian political parties / campaigns received CA pitches?
- [ ] Did any of those pitches convert to paid engagements?
- [ ] What is the connection (if any) between CA's Brazil work and the Bolsonaro campaign's WhatsApp operations?

## Sources

- [[Source HindsightFiles 2020]]
- [[HindsightFiles Analytical Index]]
- Folha de SÃ£o Paulo reporting on HindsightFiles release (January 2020)

## Vault context and open research threads

This event matters to the vault because it is either a documented deployment of CA, SCL, or successor-org methodology against a real electorate, or because it produced primary evidence (regulator action, court filing, leak, sworn testimony) that the rest of the vault depends on.

Use this node to anchor: what actually happened (chronology with dates), who was on which side (clients, operators, targets), what was deployed (data collection, microtargeting, dark ads, deepfakes, voter suppression, narrative manipulation), and what is known vs alleged vs hypothesized.

## Open research threads

- Primary sources: which court filings, regulator reports, leaked documents, or sworn statements are directly tied to this event?
- Personnel: which named individuals from the vault appeared on the operator side? Cross-link them.
- Money: how was this event funded, and through what corporate or PAC structures?
- Outcome: what was the measured (not claimed) effect on the political outcome, and what is the evidence?
- Replication: which later events used the same playbook? Link forward.

## See also

- [[Events MOC]]
- [[Master Timeline]]
- [[Vault MOC]]
