...
The first major version of Facebook's Graph API. Crucially permitted any third-party app to access **not only the app user's data, but also the full profile data of all of that user's Facebook friends** -- without those friends' knowledge or consent. This permission ("friends-data") was used by tens of thousands of apps, including [[Aleksandr Kogan]]'s "This Is Your Digital Life".

## Timeline

- **April 2014:** Facebook deprecates the friends-data API (Graph API v1.0 → v2.0)
- **April 2015:** Facebook shuts the v1.0 API entirely
- **Effect:** Data already collected by apps **remained in developers' possession** -- including the 87M-profile dataset that ended up at [[Cambridge Analytica]]

## Significance

The single technical decision that enabled the entire CA harvest. ~300 friends per average user × 270,000 app users = up to 87M profiles. [[Mark Zuckerberg]] later testified to Congress that Facebook was auditing "tens of thousands" of apps that had had similar access.
