---
date: 2026-05-16
description: Operational playbook for blocking COS in Wyoming 2026; GOP supermajority + federal-lands wedge
tags: [thread, strategy, state-playbook, state/wy, era/2024-2026]
type: thread
status: active
---

# Defensive Playbook — Wyoming

**Priority: CRITICAL.** Wyoming is on COSA's 2026 target list. **GOP supermajority = vote-margin defense impossible.** Strategy must rely on **inserting the federal-lands wedge** to peel off rural-conservative votes.

## Key actors

- **Speaker Chip Neiman** (R-1) — Freedom Caucus aligned; likely sponsor
- Wyoming has GOP supermajority in both chambers
- Governor Mark Gordon (R) — no veto backstop

## The federal-lands wedge

**This is the only viable defense in Wyoming.** Wyoming's economy is deeply tied to federal lands:
- BLM-managed grazing leases
- Federal mineral leases (oil, gas, coal, uranium)
- National parks tourism (Yellowstone, Grand Teton)
- Wyoming outdoor-rec industry

**The COSA / Williamsburg 2023 mock convention proposed transferring federal lands to states** — which sounds good in theory but in practice would:
- Force Wyoming to assume management costs ($1B+ annually)
- Transfer revenue streams to private development
- Threaten outdoor-rec tourism
- Reduce federal grazing-fee subsidies

**This is the wedge:** even MAGA Wyoming ranchers and outdoor-rec operators do NOT want federal lands transferred. The Bundy-style "states should own everything" position is fringe even in Wyoming.

## Three-track strategy

### Track 1 — Federal lands counter-coalition
Build a coalition of:
- Wyoming Outdoor Council
- Wyoming Stock Growers Association (conservative-but-pragmatic)
- Backcountry Hunters & Anglers (BHA)
- Wyoming Outfitters Association
- Theodore Roosevelt Conservation Partnership

Frame: "The same group pushing this wants to transfer federal lands to state control. That destroys our grazing leases, our hunting access, and our tourism economy."

### Track 2 — Senator Cynthia Lummis pressure
Lummis is a federal senator but Wyoming-grounded. She's been ambivalent on federal-lands transfer. Pressure her to take a public position; her stance influences state legislators.

### Track 3 — Direct outreach to Speaker Neiman's caucus
There are ~10 Wyoming legislators with deep ag / outdoor-rec ties who would lose constituent goodwill from federal-lands transfer. Brief them specifically with district-level economic data.

## Counter-narrative content for Wyoming

- **TikTok/IG/YouTube:** "What Article V Means for Wyoming Hunters" — narrated by Wyoming outfitter
- **Op-ed:** Wyoming Tribune Eagle + Casper Star-Tribune
- **Direct mail:** federal-lands-adjacent counties

## Funding ask

| Line item | Amount |
|---|---|
| Outdoor coalition coordination | $200K |
| Paid ads | $400K |
| Direct mail | $150K |
| Polling | $50K |
| Coordination + comms | $150K |
| **TOTAL 2026** | **$950K** |

## Success criteria

- Wyoming COS resolution does not pass either chamber
- Wyoming outdoor-rec industry publicly opposes
- 5+ Wyoming Republican legislators publicly raise federal-lands concerns

## Related vault notes

- [[Counter-COS Strategy for the Left]]
- [[Convention of States Action]]

## Cross-references within the vault

The Wyoming defensive playbook intersects the broader vault via the [[Article V Convention]] thread (Wyoming as a state that has both passed and (in some sessions) rescinded its Article V application), the [[Convention of States Action]] organisation, and the [[Citizens for Self-Governance]] funding map.

For peer comparisons see other state defensive playbooks (e.g. the broader [[State-Playbooks/]] folder). For the strategic context see [[Hypothesis - COS is the permanent capture arm of Project 2025]] and [[Counter-COS Strategy for the Left]].

The playbook is intentionally tactical rather than analytical; the underlying analysis lives in the lens and hypothesis notes.

## See also

- [[Article V Convention]]
- [[Convention of States Action]]
- [[Citizens for Self-Governance]]
- [[Counter-COS Strategy for the Left]]
- [[Hypothesis - COS is the permanent capture arm of Project 2025]]

---

## Master synthesis

This node is one of the structural anchors of [[Master Theses MOC]] - the vault's top-level synthesis of the three converging theses (Structural Capture / Demographic Defensiveness / Post-Liberal Constitutional Replacement) that tie the vault clusters together.
